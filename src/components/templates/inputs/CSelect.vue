<template>
  <div>
    <div
      v-if="externalLabel"
      :class="externalLabelClass ? externalLabelClass : 'mb-2 helper-text bold'"
    >
      {{ externalLabel }}
      <span class="text-negative">
        {{ required && !modelValue ? '(обязательное поле)' : '' }}</span
      >
    </div>

    <q-select
      :class="{
        'select-border-rounded': rounded,

        'cselect-row': row,
        'no-icon': noIcon,
        'default rounded': $props.default,
        'multiple-select': multiple,
      }"
      ref="currentSelect"
      :options="_items"
      :option-value="itemValue"
      :option-label="itemLabel"
      :options-dense="itemsDense"
      :option-disable="optionDisable"
      :label="label"
      :multiple="multiple"
      :readonly="readonly"
      :auto-complete="autoComplete"
      :loading="loading"
      :rules="rules"
      :name="name"
      :input-style="inputStyle"
      :input-class="inputClass"
      :disable="disabled"
      :dense="dense"
      :use-input="useInput"
      :rounded="rounded"
      :standout="standout"
      :borderless="borderless"
      :input-debounce="inputDebounce || 300"
      :filled="filled"
      :dark="dark"
      @change="$emit('change')"
      @filter="filterFn"
      @update:modelValue="$emit('update:modelValue', $event)"
      :modelValue="modelValue"
      bg-color="transparent"
      :color="color"
      :label-color="labelColor"
      :hide-selected="!multiple && useInput"
      :fill-input="!multiple && useInput"
      :map-options="emitValue"
      :emit-value="emitValue"
      :style="`height: ${height ? height : multiple ? undefined : '42px'}; ${
        width ? `width: ${width};` : ''
      }; min-height: ${multiple ? (height ? height : '42px') : 'unset'}`"
      behavior="menu"
      :stack-label="stackLabel"
      :outlined="_outlined"
      :use-chips="useChips"
      @new-value="(...e: any[]) => $emit('newValue', ...e)"
      :hide-dropdown-icon="hideDropdownIcon"
      @virtual-scroll="onVirtualScroll($event)"
    >
      <template v-if="modelValue && clearable" v-slot:append>
        <CIconButton
          icon="fal fa-times"
          @click="$emit('update:modelValue', null)"
          no-padding
          size="30"
          icon-size="16px"
          color="transparent"
          class="cursor-pointer"
        />
      </template>

      <!-- <template v-slot:option="scope">
        <q-item class="column py-6" v-bind="scope.itemProps">

          {{ itemLabel ? scope.opt[itemLabel] : scope.opt.label }}

          <template v-if="$slots.subtitle">
            <slot name="subtitle" v-bind:v="scope.opt"></slot>
          </template>
        </q-item>
      </template> -->

      <template v-slot:before-options v-if="!!$slots['before-options']">
        <slot name="before-options"></slot>
      </template>
      <template v-slot:no-option>
        <slot name="no-option"></slot>
      </template>
      <template v-slot:default>
        <slot name="default"></slot>
      </template>
      <template v-slot:selected-item="scope" v-if="$slots.selectedItem">
        <slot name="selectedItem" :scope="scope"></slot>
      </template>
      <template v-slot:selected v-if="!!$slots.selected">
        <slot name="selected"></slot>
      </template>
      <template v-slot:prepend v-if="!!$slots.prepend">
        <slot name="prepend"></slot>
      </template>
    </q-select>
  </div>
</template>

<script setup lang="ts">
import CIconButton from '../buttons/CIconButton.vue';
import { computed, ref } from 'vue';
import { QSelect } from 'quasar';
import BaseRepo from 'src/corexModels/apiModels/baseRepo';

const emit = defineEmits([
  'update:modelValue',
  'change',
  'virtualScroll',
  'click',
  'newValue',
]);

const props = defineProps<{
  labelColor?: string;
  modelValue?: any;
  color?: string;
  bgColor?: string;
  dark?: boolean;
  filled?: boolean;
  borderless?: boolean;
  standout?: boolean | string;
  rounded?: boolean;
  dense?: boolean;
  inputClass?: object | string | any[];
  inputStyle?: object | string | any[];
  name?: string;
  rules?: any[];
  loading?: boolean;
  fillInput?: boolean;
  autoComplete?: string;
  multiple?: boolean;
  items?: any[];
  itemLabel?: string;
  itemValue?: string;
  itemsDense?: boolean;
  disabled?: boolean;
  label?: string;
  height?: string;
  width?: string;
  clearable?: boolean;
  row?: boolean;
  useInput?: boolean;
  hideSelected?: boolean;
  inputDebounce?: string | number;
  emitValue?: boolean;
  stackLabel?: boolean;
  outlined?: boolean;
  default?: boolean;
  optionDisable?: string;
  noIcon?: boolean;
  filter?: (value: string) => void;
  readonly?: boolean;
  customLabels?: boolean;
  useChips?: boolean;
  externalLabel?: string;
  externalLabelClass?: string;
  hideDropdownIcon?: boolean;
  subtitle?: string;
  repo?: BaseRepo<any>;
  repoRequestParams?: Record<string, any>;
  repoSearchKey?: string;
  required?: boolean;
}>();

const currentSelect = ref<QSelect>();
const lastSearch = ref<string | null>(null);

const _items = computed(() => {
  if (props.repo) return props.repo.items as unknown[];
  return props.items;
});

const _outlined = computed(() => {
  if (!props.outlined && props.default) return true;
  return props.outlined;
});

// const _bgColor = computed(() => {
//   if (!props.bgColor && props.default) {
//     return 'white';
//   } else if (!props.bgColor && !props.default) {
//     return 'white';
//   }
//   return props.bgColor;
// });

const loadRepoItems = async (search?: string | null, page = 1) => {
  const params = props.repoRequestParams || {};
  const searchValue = search ?? undefined;
  const searchKey = props.repoSearchKey || 'search';
  if (props.repo)
    await props.repo.list(
      { [searchKey]: searchValue, ...params },
      { page, appendItems: page !== 1 }
    );
};

const onVirtualScroll = (data: {
  from: number;
  to: number;
  ref: QSelect;
  index: number;
}) => {
  if (props.repo) {
    if (
      props.repo.pagination.last_page > props.repo.pagination.page &&
      props.repo.items.length - 6 < data.index
    ) {
      void loadRepoItems(lastSearch.value, props.repo.pagination.page + 1);
    }
  }
  emit('virtualScroll', data);
};

const filterFn = (
  search: string,
  update: (func: () => void, refFunc: (ref: QSelect) => void) => void
) => {
  if (props.repo) {
    void loadRepoItems(search).then(() => {
      update(
        () => {
          lastSearch.value = search;
        },
        (ref: QSelect) => {
          ref.setOptionIndex(-1);
          ref.moveOptionSelection(1, true);
        }
      );
    });
    return;
  }
  update(
    () => {
      if (!props.filter) return;
      if (search === lastSearch.value) return;
      props.filter(search);
      lastSearch.value = search;
    },
    (ref: QSelect) => {
      ref.setOptionIndex(-1);
      ref.moveOptionSelection(1, true);
    }
  );
};
</script>

<style lang="scss">
.q-select .q-field__native.row.items-center span {
  margin-top: 0 !important;
}

.q-select:not(.q-file).q-field--float .q-field__label {
  transform: translate(0, -50%) scale(0.85) !important;
  top: 0px !important;
  padding: 0 5px;
  z-index: 1;
  transition: all 0.3s;
  width: fit-content;
}

.q-field--float .q-field__label {
  transform: translateY(-60%) scale(0.75);
}

.q-select--multiple .q-field__inner .q-field__control {
  min-height: 42px !important;
}

.cselect-row .q-field__control {
  padding: 0 0 0 7px !important;
}

.cselect-row .q-field__append {
  padding: 0 !important;
}

.q-select {
  @mixin ellipsis {
    text-overflow: ellipsis;
    white-space: nowrap;
    overflow: hidden;
  }

  &.ellipsis,
  &.ellipsis-2-lines,
  &.ellipsis-3-lines {
    overflow: unset !important;
    text-overflow: unset !important;
    white-space: unset !important;

    span {
      @include ellipsis;
    }
  }

  .ellipsis-2-lines {
    -webkit-line-clamp: 2;
  }

  .ellipsis-3-lines {
    -webkit-line-clamp: 3;
  }
}

.cselect-row .q-field__native {
  padding: 9px 0;
}

.q-select .q-field__inner {
  height: inherit;
}

.no-icon .q-field__append .q-icon.text-negative.material-icons {
  display: none;
}

.q-item.disabled {
  opacity: 1 !important;
}
</style>
